  ...
  return (
    <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-2xl cursor-pointer bg-slate-50 dark:bg-slate-900/50 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
      <div className="flex flex-col items-center justify-center pt-5 pb-6">
        ...
        <p className="mb-2 text-sm text-slate-500 dark:text-slate-400"><span className="font-semibold">{label}</span></p>
        <p className="text-xs text-slate-500 dark:text-slate-400 opacity-70">
          {multiple ? "Drag & drop or click to upload multiple files" : "Drag & drop or click to upload"}
        </p>
      </div>
      <input type="file" className="hidden" onChange={handleChange} multiple={multiple} accept={accept} />
    </label>
  );
};
